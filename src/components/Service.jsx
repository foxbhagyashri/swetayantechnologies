...
/* ---------------------------------------------------------
   Swetayan Technologies — Data Recovery (single service page)
                ...
                <div className="max-w-7xl mx-auto px-5 flex flex-col md:flex-row items-center justify-between gap-6 relative">
                    <div>
                        <h3 className="font-display font-bold text-2xl sm:text-3xl text-neutral-950 tracking-tight">
                            Lost data? Don't try another tool first.
                        </h3>
                        <p className="font-body text-neutral-900/80 mt-2">
                            Every extra read on a failing drive lowers your odds. Get a free diagnosis before doing anything else.
                        </p>
                    </div>
...
function Intro() {
    ...
    const stats = [
        { value: 15, suffix: "+", label: "Years recovering data" },
    ...
    return (
        <section className="bg-white py-20">
            <div className="max-w-7xl mx-auto px-5 grid md:grid-cols-2 gap-14 items-center">
                <Reveal>
                    <span className="font-mono text-xs tracking-[0.2em] text-orange-600">DATA RECOVERY</span>
                    <h2 className="font-display font-bold text-3xl sm:text-4xl text-neutral-900 mt-3 tracking-tight leading-tight">
                        When a drive stops responding, we get what's on it back.
                    </h2>
                    <p className="font-body text-neutral-600 mt-5 leading-relaxed">
                        Clicking hard disks, dead SSDs, degraded RAID arrays, formatted
                        partitions, or drives that have been through water or fire — our
                        recovery lab handles the full range of failure, logical and
                        physical, under clean-room-standard conditions. We diagnose first,
...
/* ---------- recovery cases (detailed) ---------- */

function RecoveryCases() {
    const cases = [
        {
            icon: HardDrive,
            title: "Hard Disk Drive (HDD) Recovery",
            desc: "Clicking, grinding, or completely silent drives — from logical corruption to head and motor faults.",
            points: ["Bad sectors & firmware corruption", "Read/write head failure", "Spindle motor & PCB faults"],
        },
        {
            icon: Layers,
            title: "SSD & NVMe Recovery",
            desc: "Controller failure and NAND-level faults on SATA, NVMe, and M.2 solid-state drives.",
            points: ["Controller & firmware failure", "TRIM/wear-levelling corruption", "Chip-off NAND extraction"],
        },
        {
            icon: Server,
            title: "RAID, NAS & Server Recovery",
            desc: "Reconstruction for degraded or failed multi-drive arrays across common RAID levels.",
            points: ["RAID 0, 1, 5, 6, 10 rebuilds", "NAS enclosures (Synology, QNAP, etc.)", "Virtual machine & server volumes"],
        },
        {
            icon: FileCheck2,
        ...
        {
            icon: UsbIcon,
            title: "External Drives, USB & Memory Cards",
            desc: "Pen drives, external HDDs/SSDs, and SD/microSD cards that won't mount or show errors.",
            points: ["Drive not detected/mounting", "Broken connectors & enclosures", "Camera & phone memory cards"],
        },
        {
            icon: Droplets,
            title: "Physical, Water & Fire Damage",
            desc: "Drives that have been dropped, submerged, or exposed to heat, handled in controlled conditions.",
            points: ["Water and moisture damage", "Fire & heat-exposed drives", "Physical impact & PCB burnout"],
        },
    ];
    return (
            ...
            <div className="max-w-7xl mx-auto px-5">
                <Reveal>
                    <div className="max-w-2xl mb-14">
                        <span className="font-mono text-xs tracking-[0.2em] text-orange-600">CASES WE HANDLE</span>
                        <h2 className="font-display font-bold text-3xl sm:text-4xl text-neutral-900 mt-3 tracking-tight">
                            Every type of failure, one lab.
                        </h2>
                    </div>
                </Reveal>
...
/* ---------- devices/media supported strip ---------- */

function MediaSupported() {
    const media = [
        { icon: Laptop, label: "Laptop drives" },
        { icon: Database, label: "Desktop HDD/SSD" },
        { icon: Server, label: "RAID / NAS arrays" },
        { icon: UsbIcon, label: "USB & pen drives" },
        { icon: Camera, label: "SD / memory cards" },
        { icon: Smartphone, label: "Phone storage" },
    ];
    return (
        <section className="bg-white py-14 border-y border-neutral-100">
            <div className="max-w-7xl mx-auto px-5">
                <Reveal>
                    <p className="font-mono text-xs tracking-[0.2em] text-neutral-400 text-center mb-8">MEDIA WE RECOVER FROM</p>
                </Reveal>
                <div className="grid grid-cols-3 md:grid-cols-6 gap-8">
                    {media.map((m, i) => (
                        <Reveal key={m.label} delay={i * 70}>
                            <div className="flex flex-col items-center text-center gap-2.5">
                                <m.icon className="w-7 h-7 text-neutral-400" />
                                <span className="font-body text-xs text-neutral-500">{m.label}</span>
                            </div>
                        </Reveal>
                    ))}
                </div>
...
function Process() {
    const steps = [
        { icon: ScanSearch, title: "Free Diagnosis", desc: "We inspect the drive and identify the actual point of failure." },
        { icon: FileCheck2, title: "Honest Quote", desc: "You get a clear price and timeline before any recovery begins." },
        { icon: Wrench, title: "Recovery", desc: "Our engineers recover the data using the appropriate method." },
...
function DoDont() {
    const donts = [
        "Keep using the drive or saving new files to it",
        "Open the drive casing or attempt DIY repair",
        "Run recovery software repeatedly on a failing drive",
    ...
    const dos = [
        ...
        "Bring or ship the drive to us as-is",
...
function Guarantees() {
    const items = [
        ...
        { icon: Clock, title: "24–48 Hour Diagnosis", desc: "Most drives are assessed within one to two working days." },
...
function FAQ() {
    const faqs = [
        { q: "How long does data recovery usually take?", a: "Diagnosis is typically done within 24–48 hours. Straightforward logical recovery often completes in 2–3 days; physically damaged drives or RAID rebuilds can take longer, which we'll confirm after diagnosis." },
        ...
        { q: "Is my data safe and confidential during recovery?", a: "Your files are never viewed, copied, or shared beyond what the recovery job requires. Drives are handled under clean-room-standard conditions throughout the process." },
        { q: "Can you recover data from a RAID or NAS system?", a: "Yes — we rebuild degraded and failed arrays across common RAID levels, including drives from NAS enclosures like Synology and QNAP." },
        { q: "Should I keep trying recovery software myself first?", a: "We'd recommend against it. Repeated read attempts on a failing drive can reduce the chance of a full recovery. Powering it down and getting a free diagnosis first gives the best odds." },
    ];
    return (
        <section className="bg-white py-20">
    ...
    return (
        <div className="font-body bg-white min-h-screen">
            ...
            <Breadcrumb
                ...
                subtitle="Hard disks, SSDs, RAID arrays, and every device in between — recovered honestly, handled with care."
                trail={[
                    { label: "Home", href: "#" },
