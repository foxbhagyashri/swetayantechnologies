...
/* ---------------------------------------------------------
   Swetayan Technologies — Hard Disk Data Recovery
...
function Breadcrumb({ trail, title, subtitle }) {
    return (
        <section className="relative bg-neutral-950 text-white overflow-hidden">
            <Img
                src="https://images.unsplash.com/photo-1531492746076-161ba9bcb065?w=1600&q=70&auto=format&fit=crop"
                alt="Open hard disk drive showing platters and read/write head"
                ...
                <div className="max-w-7xl mx-auto px-5 flex flex-col md:flex-row items-center justify-between gap-6 relative">
                    <div>
                        <h3 className="font-display font-bold text-2xl sm:text-3xl text-neutral-950 tracking-tight">
                            Hard disk clicking or not spinning up?
                        </h3>
                        <p className="font-body text-neutral-900/80 mt-2">
                            Every extra power-on attempt on a failing disk lowers your odds. Get a free diagnosis before doing anything else.
                        </p>
                    </div>
...
function Intro() {
    ...
    const stats = [
        { value: 15, suffix: "+", label: "Years recovering hard disks" },
        { value: 9800, suffix: "+", label: "HDDs recovered" },
            ...
            <div className="max-w-7xl mx-auto px-5 grid md:grid-cols-2 gap-14 items-center">
                <Reveal>
                    ...
                    <p className="font-body text-neutral-600 mt-5 leading-relaxed">
                        A hard disk drive is a mechanical device — spinning platters, a
                        read/write head, a spindle motor, and a delicate control board all
                        working in microns of tolerance. When any one of those parts fails,
                        the drive can click, grind, spin up and stop, or simply go silent.
                        Our lab handles the full range of HDD failure — logical corruption,
                        bad sectors, firmware faults, head crashes, motor seizures, and PCB
                        burnout — under clean-room-standard conditions. We diagnose first,
                ...
                <Reveal delay={150}>
                    <div className="rounded-2xl overflow-hidden mb-6 border border-neutral-200">
                        <Img
                            src="/ChatGPT Image Aug 5, 2026, 12_16_29 PM.png"
                            alt="Close-up of a hard disk drive's internal platters and actuator arm"
...
/* ---------- HDD types we recover ---------- */

function DrivesSupported() {
    const drives = [
        { img: "https://images.unsplash.com/photo-1591488320449-011701bb6704?w=500&q=70&auto=format&fit=crop", label: "Desktop 3.5\" HDDs" },
        { img: "https://images.unsplash.com/photo-1587202372775-e229f172b9d7?w=500&q=70&auto=format&fit=crop", label: "Laptop 2.5\" HDDs" },
        { img: "https://images.unsplash.com/photo-1591799264318-7e6ef8ddb7ea?w=500&q=70&auto=format&fit=crop", label: "External USB HDDs" },
        { img: "https://images.unsplash.com/photo-1591370874773-6702e8f12fd8?w=500&q=70&auto=format&fit=crop", label: "NAS enclosure disks" },
        { img: "https://images.unsplash.com/photo-1518770660439-4636190af475?w=500&q=70&auto=format&fit=crop", label: "Server & RAID disks" },
        { img: "https://images.unsplash.com/photo-1591405351990-4726e331f141?w=500&q=70&auto=format&fit=crop", label: "DVR / CCTV disks" },
    ];
    return (
...
/* ---------- HDD failure cases (detailed) ---------- */

function RecoveryCases() {
    const cases = [
        {
            icon: Volume2,
            title: "Clicking & Grinding Drives",
            desc: "The classic \"click of death\" — usually a head or actuator fault, not something to keep powering on.",
            points: ["Head assembly misalignment", "Actuator arm failure", "Stuck or seized spindle"],
        },
        {
            icon: Disc3,
            title: "Head Crash & Platter Damage",
            desc: "Physical contact between the read/write head and platter surface, handled in a controlled environment.",
            points: ["Scratched or scored platters", "Damaged read/write heads", "Donor-head swaps when needed"],
        },
        {
            icon: CircuitBoard,
            title: "PCB & Motor Failure",
            desc: "A burnt controller board or seized spindle motor can make a healthy disk look completely dead.",
            points: ["Power surge & burnt PCB", "Spindle motor seizure", "Firmware module corruption"],
        },
        {
            icon: FileCheck2,
            title: "Bad Sectors & Firmware Corruption",
            desc: "Drives that are detected but slow, freezing, or throwing read errors on specific files.",
            points: ["Growing bad sector count", "Firmware/service area corruption", "Slow or intermittent detection"],
        },
        {
            icon: AlertTriangle,
            title: "Formatted, Deleted & Corrupted Data",
            desc: "Recovery after accidental formatting, deletion, or a partition table that's gone missing.",
            points: ["Accidental format or repartition", "Deleted files & folders", "Corrupted NTFS/exFAT/ext file systems"],
        },
        {
            icon: Flame,
            title: "Water, Fire & Physical Damage",
            desc: "Disks that have been dropped, submerged, or exposed to heat, opened and assessed under clean-room-standard conditions.",
            points: ["Water and moisture damage", "Fire & heat-exposed disks", "Drop impact & bent housings"],
        },
    ];
    return (
            ...
            <div className="max-w-7xl mx-auto px-5">
                <Reveal>
                    <div className="max-w-2xl mb-14">
                        <span className="font-mono text-xs tracking-[0.2em] text-orange-600">HDD FAILURES WE HANDLE</span>
                        <h2 className="font-display font-bold text-3xl sm:text-4xl text-neutral-900 mt-3 tracking-tight">
                            Every kind of hard disk failure, one lab.
                        </h2>
                    </div>
                </Reveal>
...
function Process() {
    const steps = [
        { icon: ScanSearch, title: "Free Diagnosis", desc: "We inspect the disk and identify the actual point of failure — mechanical, electronic, or logical." },
...
function DoDont() {
    const donts = [
        "Keep powering the disk on to \"see if it still works\"",
        "Open the drive casing or attempt DIY head/platter repair",
        "Run recovery software repeatedly on a clicking or grinding disk",
        "Freeze the drive — it doesn't help, and often makes things worse",
    ];
    const dos = [
        "Power the disk down the moment you notice clicking, grinding, or errors",
        "Note down what happened right before the issue started",
        "Bring or ship the disk to us as-is",
...
function Guarantees() {
    const items = [
        ...
        { icon: Clock, title: "24–48 Hour Diagnosis", desc: "Most disks are assessed within one to two working days." },
...
function FAQ() {
    const faqs = [
        { q: "My hard disk is clicking — is it still safe to use?", a: "No. Clicking almost always means a mechanical fault with the head or actuator. Continuing to power the disk on risks scratching the platters and turning a recoverable fault into an unrecoverable one. Power it down and get a free diagnosis." },
        { q: "How long does hard disk data recovery usually take?", a: "Diagnosis is typically done within 24–48 hours. Straightforward logical recovery often completes in 2–3 days; physically damaged disks needing a donor head or platter work can take longer, which we'll confirm after diagnosis." },
        { q: "What happens if my data can't be recovered?", a: "You only pay for the diagnosis, not the recovery fee. We'll always tell you upfront if a disk isn't recoverable rather than attempting work we don't expect to succeed." },
        { q: "Can you recover a hard disk that's making no sound at all?", a: "Yes. A silent disk often points to a seized spindle motor or a failed PCB rather than platter damage, both of which we can frequently work around in the lab." },
        { q: "Should I try recovery software myself first?", a: "We'd recommend against it, especially if the disk is making unusual noises. Repeated read attempts on a failing disk can reduce the chance of a full recovery. Powering it down and getting a free diagnosis first gives the best odds." },
    ];
    return (
        <section className="bg-white py-20">
...
export default function HardDiskDataRecoveryPage() {
    return (
